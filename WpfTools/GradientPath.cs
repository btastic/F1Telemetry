    ...
    public class GradientPath : FrameworkElement
    {
        ...
        protected override void OnRender(DrawingContext drawingContext)
        {
            ...
            foreach (PathFigure pathFigure in pathGeometry.Figures)
            {
                ...
                DrawLineCap(drawingContext, pathFigure.StartPoint, vector, StrokeStartLineCap, PenLineCap.Flat);
                ...
                for (int index = 0; index < points.Count; index++)
                {
                    ...
                    {
                        StartPoint = rotate.Transform(ptUpPrev),
                        IsClosed = true,
                        IsFilled = true
                    };
                    ...
                    drawingContext.DrawGeometry(brush, pen, tetragonGeo);
                    drawingContext.Pop();

                    // Something special for the last point
                    if (index == points.Count - 1)
                    {
                        DrawLineCap(drawingContext, point, -vect1, PenLineCap.Flat, StrokeEndLineCap);
                    }
